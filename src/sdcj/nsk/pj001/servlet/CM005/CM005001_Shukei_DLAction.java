...
/**
 * Servlet implementation class CM005001_Shukei_DLAction
 */
@WebServlet("/CM005001_Shukei_DLAction")
public class CM005001_Shukei_DLAction extends HttpServlet {
	private static final long serialVersionUID = 1L;

    /**
     * @see HttpServlet#HttpServlet()
     */
    public CM005001_Shukei_DLAction() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 入力 OK
		try {
			var shukeiList = UridenJViewDao.selectCSV3ByCondition(slipNo_min,slipNo_max,salesDate_min,salesDate_max,customer_min,customer_max,tantou_min,tantou_max);
			if (shukeiList.size() == 0) {
				// a 件数が0
				// MSG001
				...
				// ②CSVファイルの作成・編集
				// a ファイル名
				String meisho = CsvUtil.getCsvName("CSV004");
				// 現在日時を取得
				...
				// b 出力項目
				String outputString = "";
				for (var shukei : shukeiList) {
					outputString +=
							"\"" + sdf2.format(timestamp) + "\"" + ","
							+ "\"" + shukei.getTokuiCode_001() + "\"" + ","
							+ "\"" + shukei.getTokuiName_003() + "\"" + ","
							+ "\"" + shukei.getShukeiNengetsu() + "\"" + ","
							+ "\"" + shukei.getKingakuGoukei() + "\""
