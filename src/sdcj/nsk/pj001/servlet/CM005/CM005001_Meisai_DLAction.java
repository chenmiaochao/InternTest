...
/**
 * Servlet implementation class CM005001_Meisai_DLAction
 */
@WebServlet("/CM005001_Meisai_DLAction")
public class CM005001_Meisai_DLAction extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public CM005001_Meisai_DLAction() {
		super();
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			var meisaiList = UridenJViewDao.selectCSV2ByCondition(slipNo_min,slipNo_max,salesDate_min,salesDate_max,customer_min,customer_max,tantou_min,tantou_max);
			...
			if (meisaiList.size() == 0) {
				...
				// ②CSVファイルの作成・編集
				// a ファイル名
				String meisho = CsvUtil.getCsvName("CSV003");
				// 現在日時を取得
				var timestamp = new Timestamp(System.currentTimeMillis());
				var sdf1 = new SimpleDateFormat("yyyyMMddhhmmss");
				var sdf2 = new SimpleDateFormat("yyyyMMdd");
				String fileName = URLEncoder.encode(meisho + "_" + sdf1.format(timestamp), CHARSET_UTF8) + ".csv";

				// b 出力項目
				String outputString = "";
				for (var meisai : meisaiList) {
					outputString += "\"" + meisai.getTokuiCode_001() + "\"" + ","
							+ "\"" + meisai.getTokuiName_003() + "\"" + ","
							+ "\"" + sdf2.format(timestamp) + "\"" + ","
							+ "\"" + meisai.getKingakuGoukei() + "\"" + ","
							+ "\"" + meisai.getDenNo_001() + "\"" + ","
							+ "\"" + meisai.getUriDate_001() + "\"" + ","
							+ "\"" + meisai.getMeisaiNo_002() + "\"" + ","
							+ "\"" + meisai.getShohiCode_002() + "\"" + ","
							+ "\"" + meisai.getShohinName_002() + "\"" + ","
							+ "\"" + meisai.getSuryo_002() + "\"" + ","
							+ "\"" + meisai.getTanka_002() + "\"" + ","
							+ "\"" + meisai.getKingaku_002() + "\"" + ","
