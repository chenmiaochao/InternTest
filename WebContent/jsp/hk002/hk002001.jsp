            ...
            <td>
             ...
             <table border="0" cellspacing="2" cellpadding="0" width="100%">
              ...
              <tr>
               ...
               <td align="center">&gt;&gt;</td>
               <td align="left"><b>得意先選択 補助検索画面</b></td>
               <td></td>
              </tr>
             </table>
...
-->
        ...
        <!-- 画面固有UI begin ここまで -->
             ...
             <table width="100%">
                 ...
                 <tr>
                  <td>
                   <table border="1" cellspacing="0" cellpadding="1" bordercolor="#888888">
                    <tr>
                     <td>
                     <form action="<%=request.getContextPath()%>/HK002001_SearchAction" method="POST">
                      <table border="0" cellspacing="0" cellpadding="5" width="540" height="150">
                       ...
                       <tr>
                        ...
                        <td></td>
                        <td>
                         <input type="text" size="20" name="startCode" value="${requestScope.STARTCODE }"/>
                         &nbsp;～&nbsp;
                         <input type="text" size="20" name="endCode" value="${requestScope.ENDCODE }"/>
                        </td>
                       </tr>
                       <!-- 得意先名 -->
                       <tr>
                        ...
                        <td></td>
                        <td>
                         <input name="tokuiName" type="text" size="40" value="${requestScope.TOKUINAME }"/>
                        </td>
                       </tr>
                       <!-- ボタン -->
                       <tr height="50">
                        ...
                        <td>
                         ...
                         <input type="button" value="　クリア　" onclick="location.href='<%=request.getContextPath()%>/HK002001_ClearAction'"/>
                         <input type="button" value="　閉じる　" onclick="location.href='<%=request.getContextPath()%>/HK002001_CloseAction'"/>
                        </td>
                       </tr>
                      </table>
                      </form>
              ...
              <c:set var ="tokuiLengthErr" value ="${requestScope.TOKUINAMEERR }"/>
              ...
              <c:if test="${ not empty tokuiLengthErr}">
              		<h3><c:out value="${ tokuiLengthErr}"></c:out> </h3>
              </c:if>
                 	...
                 	<td>
                 		<c:choose>
                 			<c:when test="${currentPage eq 1 }">
                 				&lt;&lt;前へ
                 				<a href="<%=request.getContextPath() %>/HK002001_PageChangeAction?page=${currentPage +1 }&startCode=${requestScope.STARTCODE }&endCode=${requestScope.ENDCODE }&tokuiName=${requestScope.TOKUIENCODE }">次へ&gt;&gt;</a> |
	                 				<c:out value="${size }"></c:out>件中
	                 		 	<c:out value="1"></c:out>～<c:out value="10"></c:out> 件
                 			</c:when>
                 			<c:when test="${currentPage eq lastPage }">
								<a href="<%=request.getContextPath() %>/HK002001_PageChangeAction?page=${currentPage -1 }&startCode=${requestScope.STARTCODE }&endCode=${requestScope.ENDCODE }&tokuiName=${requestScope.TOKUIENCODE }">&lt;&lt;前へ</a> |
	                 			次へ&gt;&gt; |
	                 				<c:out value="${size }"></c:out>件中
						...
						<c:if test="${size le 50 }">
							<c:forEach begin = "1" end = "${lastPage }" varStatus="loop">
                				<c:choose>
                					...
                					<c:otherwise>
										<a href="<%=request.getContextPath() %>/HK002001_PageChangeAction?page=${loop.index }&startCode=${requestScope.STARTCODE }&endCode=${requestScope.ENDCODE }&tokuiName=${requestScope.TOKUIENCODE }"><c:out value="${loop.index }"></c:out></a>
									</c:otherwise>
								</c:choose>
							</c:forEach>
						</c:if>
                 ...
                 <tr>
                  <td>
                   <table border="1" cellspacing="0" cellpadding="1" bordercolor="#888888">
                    ...
                    <tr bgcolor="#AAAAAA">
                     <th>操作</th>
                     <th width="200">得意先コード</th>
                     <th width="550">得意先名</th>
                    </tr>
                    <!-- 明細 -->
                    <c:forEach var="rows" items="${list}">
                    <form action="<%=request.getContextPath() %>/HK002001_SelectAction" method="post">

                    <tr>
                     <td><input type="submit" value="　選択　" /></td>
					 <input type="hidden"/ name="tokuiCode" value ="${rows.tokuiCode }">
					 <input type="hidden"/ name="tokuiName" value ="${rows.tokuiName }">
                     <td>${rows.tokuiCode }</td>
                     <td>${rows.tokuiName }</td>
                   </tr>
                   </form>
                   </c:forEach>
                   </table>
