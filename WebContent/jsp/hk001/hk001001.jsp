...
-->
        ...
        <!-- 画面固有UI begin ここまで -->
             ...
             <table width="100%">
                 ...
                 <tr>
                  <td>
                   <table border="1" cellspacing="0" cellpadding="1" bordercolor="#888888">
                    <tr>
                     <td>
                     <form action="<%=request.getContextPath()%>/HK001001_SearchAction" method="POST">
                      <table border="0" cellspacing="0" cellpadding="5" width="540" height="150">
                       ...
                       <tr>
                        ...
                        <td></td>
                        <td>
                         <input name="shohinName" type="text" size="40" value="${requestScope.SHOHINNAME }">
                        </td>
                       </tr>
                       <!-- ボタン -->
                       <tr height="50">
                        ...
                        <td>
                         ...
                         <input type="button" value="　クリア　" onclick="location.href='<%=request.getContextPath()%>/HK001001_ClearAction'"/>
                         <input type="button" value="　閉じる　" onclick="location.href='<%=request.getContextPath()%>/HK001001_CloseAction'"/>
              ...
              <c:if test="${ not empty shohinLengthErr}">
                 	...
                 	<td>
                 		<c:choose>
                 			...
                 			<c:when test="${currentPage eq 1 }">
                 				&lt;&lt;前へ |
                 				<a href="<%=request.getContextPath() %>/HK001001_PageChangeAction?page=${currentPage +1 }&startCode=${requestScope.STARTCODE }&endCode=${requestScope.ENDCODE }&shohinName=${requestScope.NAMEENCODE }">次へ&gt;&gt;</a> |
	                 				<c:out value="${size }"></c:out>件中
	                 		 	<c:out value="1"></c:out>～<c:out value="10"></c:out> 件
                 			</c:when>
                 			<c:when test="${currentPage eq lastPage }">
								<a href="<%=request.getContextPath() %>/HK001001_PageChangeAction?page=<c:out value="${currentPage -1 }"/>&startCode=${requestScope.STARTCODE }&endCode=${requestScope.ENDCODE }&shohinName="${requestScope.NAMEENCODE}">&lt;&lt;前へ</a> |
	                 			次へ&gt;&gt; |
	                 				<c:out value="${size }"></c:out>件中
	                 			 <c:out value="${requestScope.NUMBERFIRST }"></c:out>～<c:out value="${requestScope.NUMBERLAST }"></c:out> 件
                 			</c:when>
                 			<c:otherwise>
                 				<a href="<%=request.getContextPath() %>/HK001001_PageChangeAction?page=${currentPage -1 }&startCode=${requestScope.STARTCODE }&endCode=${requestScope.ENDCODE }&shohinName=${requestScope.NAMEENCODE }">&lt;&lt;前へ</a> |
	                 			<a href="<%=request.getContextPath() %>/HK001001_PageChangeAction?page=${currentPage +1 }&startCode=${requestScope.STARTCODE }&endCode=${requestScope.ENDCODE }&shohinName=${requestScope.NAMEENCODE }">次へ&gt;&gt;</a> |
	                 				<c:out value="${size }"></c:out>件中
	                 			 <c:out value="${requestScope.NUMBERFIRST }"></c:out>～<c:out value="${requestScope.NUMBERLAST }"></c:out> 件
                 			</c:otherwise>
						...
						<c:if test="${size le 50 }">
							<c:forEach begin = "1" end = "${lastPage }" varStatus="loop">
                				<c:choose>
                					...
                					<c:otherwise>
										<a href="<%=request.getContextPath() %>/HK001001_PageChangeAction?page=${loop.index }&startCode=${requestScope.STARTCODE }
							...
							<c:if test="${currentPage lt 4 }">
									<c:forEach begin = "1" end = "6" varStatus="loop">
										<c:choose>
		                					...
		                					<c:otherwise>
												<a href="<%=request.getContextPath() %>/HK001001_PageChangeAction?page=${loop.index }&startCode=${requestScope.STARTCODE }&endCode=${requestScope.ENDCODE }&shohinName=${requestScope.NAMEENCODE }"><c:out value="${loop.index }"></c:out></a>
											</c:otherwise>
										</c:choose>
									</c:forEach>
							</c:if>
                 ...
                 <tr>
                  <td>
                   ...
                   <table border="1" cellspacing="0" cellpadding="1" bordercolor="#888888">
                    ...
                    <form action="<%=request.getContextPath() %>/HK001001_SelectAction" method="post">
                    <tr>

                    <input type="hidden" name="shohinCode" value ="${rows.shohinCode }">
                    <input type="hidden" name="shohinName" value ="${rows.shohinName }">
                    <input type="hidden" name="tanka" value ="${rows.tanka }">
